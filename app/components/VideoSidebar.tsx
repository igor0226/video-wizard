...
function StatusIcon({ status }: { status: VideoItem["status"] }) {
	...
	if (status === "processing") {
		return (
			<Loader2
				className="h-3.5 w-3.5 animate-spin text-zinc-300"
				aria-hidden="true"
			/>
		);
	}
		...
		<aside className="videoSidebar">
			...
			<ScrollArea className="h-[calc(78vh-92px)]">
				<ul className="space-y-1.5 pr-2">
					...
					{videos.length === 0 ? (
						<li className="rounded-lg border border-dashed border-zinc-700 p-3 text-xs text-zinc-400">
							No videos yet. Upload one to start.
						</li>
					) : null}
				</ul>
			</ScrollArea>
