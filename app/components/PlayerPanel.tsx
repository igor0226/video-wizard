...
export function PlayerPanel({
  ...
  const clampedCurrentTime = Math.max(0, Math.min(currentTime, totalDuration || 0));
    ...
    <section className="playerPanel">
      ...
      {selectedVideo && selectedVideo.status !== "ready" ? (
        <p className="note">
          ...
          {selectedVideo.failureReason ? ` - ${selectedVideo.failureReason}` : ""}
        </p>
      ) : null}
      <div className="flex items-center gap-2 mt-4">
        <Button type="button" className="playPauseButton" onClick={onTogglePlay} disabled={!selectedVideo?.playable}>
          {isPlaying ? (
