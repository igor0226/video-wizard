...
function parseIsoDurationSeconds(value: string | null): number {
	...
	const match =
		/^P(?:\d+Y)?(?:\d+M)?(?:\d+D)?(?:T(?:(\d+(?:\.\d+)?)H)?(?:(\d+(?:\.\d+)?)M)?(?:(\d+(?:\.\d+)?)S)?)?$/.exec(
			value.trim(),
		);
	if (!match) {
		return 0;
...
function resolveNumberTemplate(template: string, value: number): string {
	return template.replace(
		/\$Number(?:%0(\d+)d)?\$/g,
		(_full, widthText: string | undefined) => {
			if (!widthText) {
				return String(value);
			}
...
export function findSegmentIndexByTime(
	...
	for (let index = 0; index < segments.length; index += 1) {
		...
		if (
			target < segment.startSeconds + segment.durationSeconds ||
...
export function getBufferedUntil(
...
export function parseDashManifest(
	...
	const timescale =
		Number.parseInt(segmentTemplate.getAttribute("timescale") ?? "1", 10) || 1;
	const startNumber =
		Number.parseInt(segmentTemplate.getAttribute("startNumber") ?? "1", 10) ||
		1;
	const durationTicks =
		Number.parseInt(segmentTemplate.getAttribute("duration") ?? "0", 10) || 0;

	const timelineEntries = segmentTemplate.querySelectorAll(
		"SegmentTimeline > S",
	);
	...
	const initializationPath = initializationTemplate.replace(
		/\$RepresentationID\$/g,
		representationId,
	);
	const segments: DashSegment[] = timelineValues.map((segment, index) => {
		const mediaPath = resolveNumberTemplate(
			mediaTemplate
				...
				.replace(/\$Time\$/g, String(segment.start)),
			startNumber + index,
		);
		return {
			index,
