...
import type { BlobStorage, CreateVideoInput, VideoRecord, VideoRepository } from "./types";
...
const localVideoRepository: VideoRepository = {
  async createVideo(input: CreateVideoInput) {
    ...
    const sourceRelativePath = path.posix.join(UPLOADS_DIR, videoId, safeFileName);
    ...
    await localBlobStorage.writeUploadFile(sourceRelativePath, input.fileBuffer);
    const recordPath = localBlobStorage.resolveRelativePath(path.posix.join(RECORDS_DIR, toRecordFileName(videoId)));
...
export function getStoragePathsForVideo(video: VideoRecord): {
  ...
  const dashRelativeManifest = path.posix.join(video.dashRelativePath, video.manifestFileName);
  return {
    sourceAbsolutePath: localBlobStorage.resolveRelativePath(video.sourceRelativePath),
    dashAbsolutePath: localBlobStorage.resolveRelativePath(video.dashRelativePath),
    manifestAbsolutePath: localBlobStorage.resolveRelativePath(dashRelativeManifest)
  };
}
