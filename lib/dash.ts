...
export async function readDashManifest(videoId: string): Promise<DashManifest> {
  ...
  const manifestRelativePath = path.posix.join(video.dashRelativePath, video.manifestFileName);
...
export async function resolveDashAssetPath(videoId: string, assetPathParts: string[]): Promise<string> {
  ...
  const normalizedParts = assetPathParts.map((part) => decodeURIComponent(part)).filter(Boolean);
