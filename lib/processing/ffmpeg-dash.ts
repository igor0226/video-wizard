...
export async function generateDashAssets(video: VideoRecord): Promise<{ segmentCount: number }> {
  await blobStorage.ensureLayout();
  const { sourceAbsolutePath, dashAbsolutePath, manifestAbsolutePath } = getStoragePathsForVideo(video);
  ...
  try {
    console.info(`[video-worker] stage=ffmpeg-run videoId=${video.id} output="${manifestAbsolutePath}"`);
  ...
  const segmentCount = files.filter((entry) => entry.isFile() && path.extname(entry.name).toLowerCase() === ".m4s").length;
  return { segmentCount };
}
